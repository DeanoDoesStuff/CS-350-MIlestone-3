...
## for a dash.red.blink(on_time=0.5, off_time=0, n=1, background=False)
##
...
class CWMachine(StateMachine):
    ...
    print("doDot transition defined") # temp print statement
    ...
    ##
    ## run - kickoff the trasmit functionality in a separate excecution thread
    ##
    def run(self):
    ...
    ##
    ## trasmit - utility method used to continuously send a message
    ##
    def transmit(self):
        ##
        ...
        while not self.endTransmission:
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    ## Setup counter to determine time buffer after letters
                    print(f'Character: {char}, Morse: {morse}')
                    lenMorse = len(morse)
                    morseCounter = 1
                    for x in morse:
                        ...
                        ## Dot or dash?
                        if x == '.':
                            self.send('doDot') ## Transition to the dot state
                            self.send('doDot')
                        elif x == '-':
                            print('Triggering doDash event')
                            self.send('doDash') ## Transition to the dash state
                            self.send('doDash')
                        # If we are still sending process a dotDashPause event
                        morseCounter += 1
                        if morseCounter < lenMorse:
                            self.send('doDDP') ## Transition to the dotDashPause state
                            self.send('doDDP')

                    # If we are still sending process a letterPause event
                    if wordCounter < lenWord:
                        self.send('doLP') ## Transition to the letterPause state
                        self.send('doLP')
                        wordCounter += 1

                # If we are still sending process a wordPause event
                if wordsCounter < lenWords:
                    self.send('doWP') ## Transition to the wordPause state
                    self.send('doWP')
                    wordsCounter += 1
...
greenButton.when_pressed = cwMachine.processButton ## Assign the appropriate function to the greenButton variable
##
## Setup loop variable
##
repeat = True
